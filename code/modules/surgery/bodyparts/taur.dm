...
/obj/item/bodypart/taur
	...
	var/taur_markings_state
	var/taur_tertiary_state
	...
	/// Ratwood barding family: mammalian, reptilian, or deer.
...
/obj/item/bodypart/taur/get_limb_icon(dropped, hideaux = FALSE)
	...
	var/icon/markings_icon
	if(taur_markings_state)
		markings_icon = icon(icon, taur_markings_state, image_dir)
		if(has_taur_color && taur_markings)
			markings_icon.Blend(taur_markings, color_blend_mode)
	var/icon/tertiary_icon
	if(taur_tertiary_state)
		tertiary_icon = icon(icon, taur_tertiary_state, image_dir)
		if(has_taur_color && taur_tertiary)
			tertiary_icon.Blend(taur_tertiary, color_blend_mode)
	...
	. += working
	if(markings_icon)
		var/image/markings = image(markings_icon)
		markings.layer = -BODY_ADJ_LAYER
		markings.pixel_x = offset_x
		. += markings
	if(tertiary_icon)
		var/image/tertiary = image(tertiary_icon)
