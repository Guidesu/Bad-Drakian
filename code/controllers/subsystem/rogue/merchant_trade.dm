...
/datum/controller/subsystem/merchant_trade/proc/force_auto_hail()
	...
	hails_by_realm[picked.realm_id] = (hails_by_realm[picked.realm_id] || 0) + 1
	scom_announce("The [picked.ship_type] [picked.ship_name] has sailed into the docks of [get_realm_name()] unbidden, hoping to find a buyer.")
	broadcast_market_change()
	return "ok"
...
/datum/controller/subsystem/merchant_trade/proc/announce_dock(datum/trade_ship/ship)
	...
	scom_announce("The [ship.ship_type] [ship.ship_name], flying the colors of [realm_name], has made port at the docks of [get_realm_name()].")
