SUBSYSTEM_DEF(investments)
	name = "investments"
	wait = 1 MINUTES 
	flags = SS_KEEP_TIMING
	var/max_available_investments = 20
	var/list/available_investments = list()
	var/list/awaiting_investments = list()
	var/list/active_investments = list()
	var/fire_num_before_regen = 10
	var/fire_num_before_hard = 15
	var/generated_through_round = 0

/datum/controller/subsystem/investments/Initialize(start_timeofday)
	regenerate_investments()
	. = ..()

/datum/controller/subsystem/investments/fire(resumed = 0)
	process_investments()
	if(times_fired % fire_num_before_regen) 
		regenerate_investments()
	else if(times_fired % fire_num_before_hard)
		regenerate_investments(TRUE)


/datum/controller/subsystem/investments/proc/get_treasury_rate()
	if(generated_through_round <= 2500)
		return 1

	if(generated_through_round >= 15000)
		return 0

	if(generated_through_round >= 5000)
		return 1 - ((generated_through_round - 5000) / (15000 - 5000))

	return 1


/datum/controller/subsystem/investments/proc/get_investment_rate()
	var/count = length(active_investments)
	if(count < 2)
		return 2

	if(count <= 7)
		return 2 - ((count - 2) / (7 - 2))

	if(count <= 20)
		return 1 - ((count - 7) * (0.75 / (20 - 7)))

	return 0.25

/datum/controller/subsystem/investments/proc/regenerate_investments(force_reload = FALSE)
	if(force_reload)
		for(var/datum/investment/I in available_investments)
			qdel(I)
		available_investments.Cut()

	if(available_investments.len < max_available_investments)
		var/list/datum/investment/investments = subtypesof(/datum/investment)
		var/diff = max_available_investments - available_investments.len
		for(var/i = 1, i<diff, ++i)
			var/random_investment = pick(investments)
			var/datum/investment/investment = new random_investment
			if(investment.random_values)
				investment.price = floor(investment.price * (rand(80, 130) / 100))

				if(investment.onetime_payment > 0)
					investment.onetime_payment = max(floor(investment.onetime_payment * (rand(90, 130) / 100)),investment.price+5)

				investment.pay_eta = floor(investment.pay_eta * (rand(50, 150) / 100))
				investment.fail_chance = floor(investment.fail_chance * (rand(80, 120) / 100))

				investment.regular_payment = ceil(investment.regular_payment * (rand(80, 110) / 100))

			available_investments += investment
	else 
		return FALSE
	return TRUE

/datum/controller/subsystem/investments/proc/purchase_investment(datum/investment/investment)
	if(investment in available_investments)
		if(!SStreasury.withdraw_money_treasury(investment.price))
			return FALSE
		awaiting_investments += investment
		investment.time_purchased = world.time
		available_investments -= investment
	else 
		return FALSE

	return TRUE


/datum/controller/subsystem/investments/proc/process_investments()
	var/money_earned = 0
	for(var/datum/investment/investment in awaiting_investments)
		if(world.time >= investment.time_purchased + investment.pay_eta)
			if(investment.fail_chance > rand(1,100))
				SStreasury.log_to_steward("Investment '[investment.investment_name]' failed. Money lost.")
				SStreasury.steward_machine.say("Investment '[investment.investment_name]' failed! Money lost.")
				awaiting_investments -= investment
				qdel(investment)
			else
				if(investment.regular_payment)
					active_investments += investment
					awaiting_investments -= investment
					SStreasury.steward_machine.say("Investment '[investment.investment_name]' is successful, income should increase.")
				else
					SStreasury.steward_machine.say("We received a one-time payment for investment '[investment.investment_name]'.")
					money_earned += investment.onetime_payment
					awaiting_investments -= investment
					qdel(investment)

	var/treasury_rate = get_treasury_rate()
	var/invest_rate = get_investment_rate()
	for(var/datum/investment/investment in active_investments)
		money_earned += investment.regular_payment * treasury_rate * invest_rate
		generated_through_round += investment.regular_payment

	if(money_earned != 0)
		SStreasury.give_money_treasury(money_earned, "Investments")
		SStreasury.steward_machine.say("Received [money_earned]m for investments.")



/////DATUMS////
/datum/investment
	var/investment_name = "Investment"
	var/pay_eta = 15 MINUTES
	var/time_purchased
	var/price = 0
	var/onetime_payment = 0
	var/regular_payment = 0
	var/fail_chance = 0
	var/random_values = TRUE

/datum/investment/land
	investment_name = "Investment in land"
	price = 1000
	pay_eta = 5 MINUTES
	regular_payment = 34
	fail_chance = 7

/datum/investment/real_estate
	investment_name = "Investment in real estate"
	price = 2500
	pay_eta = 12.5 MINUTES
	regular_payment = 84
	fail_chance = 7

/datum/investment/trade_routes
	investment_name = "Investment in trade routes"
	price = 8000
	pay_eta = 20 MINUTES
	regular_payment = 267
	fail_chance = 7

/datum/investment/repair_bridge
	investment_name = "Finance the repair of trade routes"
	price = 100
	pay_eta = 7.5 MINUTES
	regular_payment = 4
	fail_chance = 5

/datum/investment/trade_loan
	investment_name = "Loan to the trade guild"
	price = 200
	pay_eta = 5 MINUTES
	onetime_payment = 300
	fail_chance = 20

/datum/investment/trade_loan_merc
	investment_name = "Mercenary guild loan"
	price = 100
	pay_eta = 5 MINUTES
	onetime_payment = 120
	fail_chance = 20

/datum/investment/trade_loan_unknown
	investment_name = "Urgent loan to an unreliable person"
	price = 300
	pay_eta = 5 MINUTES
	onetime_payment = 900
	fail_chance = 50

/datum/investment/gold
	investment_name = "Investment in gold"
	price = 45
	pay_eta = 2 MINUTES
	onetime_payment = 60
	fail_chance = 30

/datum/investment/silver
	investment_name = "Investment in silver"
	price = 60
	pay_eta = 2 MINUTES
	onetime_payment = 90
	fail_chance = 30

/datum/investment/gems
	investment_name = "Investment in precious stones"
	price = 60
	pay_eta = 2 MINUTES
	onetime_payment = 70
	fail_chance = 30

/datum/investment/gold_x5
	investment_name = "Investment in gold x5"
	price = 225
	pay_eta = 3 MINUTES
	onetime_payment = 300
	fail_chance = 30

/datum/investment/silver_x5
	investment_name = "Silver Investment x5"
	price = 300
	pay_eta = 3 MINUTES
	onetime_payment = 450
	fail_chance = 30

/datum/investment/gems_x5
	investment_name = "Gems Investment x5"
	price = 225
	pay_eta = 3 MINUTES
	onetime_payment = 350
	fail_chance = 30

/datum/investment/feodal_lands
	investment_name = "Local Fief Investment"
	price = 800
	pay_eta = 7.5 MINUTES
	regular_payment = 27
	fail_chance = 3

/datum/investment/overseas_feodal_lands
	investment_name = "Overseas Fief Investment"
	price = 850
	pay_eta = 7.5 MINUTES
	regular_payment = 29
	fail_chance = 10

/datum/investment/salt_mine
	investment_name = "Salt Mine Share"
	price = 1500
	pay_eta = 15 MINUTES
	regular_payment = 50
	fail_chance = 4

/datum/investment/blacksmith_guild
	investment_name = "Blacksmith Guild Support"
	price = 3000
	pay_eta = 10 MINUTES
	regular_payment = 100
	fail_chance = 6

/datum/investment/river_ferry
	investment_name = "Construction of a river crossing"
	price = 1200
	pay_eta = 8 MINUTES
	regular_payment = 40
	fail_chance = 5

/datum/investment/alchemist_lab
	investment_name = "Sponsoring alchemical research"
	price = 900
	pay_eta = 5 MINUTES
	regular_payment = 36
	fail_chance = 25

/datum/investment/smugglers_cargo
	investment_name = "Redemption of confiscated goods from smugglers"
	price = 1200
	pay_eta = 15 MINUTES
	onetime_payment = 2800
	fail_chance = 40
