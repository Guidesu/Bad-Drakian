...
/proc/bountychoice_hereticspy(mob/living/carbon/human/H)
	...
	if(H.patron?.type == /datum/patron/inhumen/baotha)
		...
		switch(crimeschoice_baotha)
			if("I'm nobody")
				to_chat(H, span_purple("I hid well, but I also trained poorly..."))
			if("They fear me")
				if(HAS_TRAIT(H, TRAIT_PSYDONIAN_GRIT))
...
/proc/bountychoice_blood_raider(mob/living/carbon/human/H)
	var/crimes = list("I'm nobody", "They fear me")
	var/crimeschoice = input(H, "Who is me", "How much have I done?") as anything in crimes
	if(istype(H.dna.species, /datum/species/elf/dark))
		switch(crimeschoice)
			if("I'm nobody")
				to_chat(H, span_warning("My past life haunts me at night. Nightmares make me look back more often..."))
			if("They fear me")
				wretch_select_bounty(H)
				ADD_TRAIT(H, TRAIT_ANTHRAXI, TRAIT_GENERIC)
				H.change_stat(STATKEY_SPD, 1)
				H.change_stat(STATKEY_PER, 1)
				to_chat(H, span_warning("They are afraid of me. My dexterity and vigilance have never failed me in the raids of vile foreigners."))
	else
		switch(crimeschoice)
			if("I'm nobody")
				to_chat(H, span_warning("Anthrax are coming for my soul, I can’t be sure of the future..."))
			if("They fear me")
				wretch_select_bounty(H)
				H.change_stat(STATKEY_WIL, 1)
				H.change_stat(STATKEY_CON, 1)
				to_chat(H, span_warning("Having adapted to the new weapon, it became easier for me to avoid head hunters."))
